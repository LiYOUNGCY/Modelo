...
class User extends Model
{
    ...
    {
        $attributes['can_qrcode'] = false;
        $attributes['can_buy'] = true;
        return parent::create($attributes); // TODO: Change the autogenerated stub
    }

    public static function getChildren($userId)
    {
        $result = DB::table('user_relation')
            ->where('user_relation.parent_id', $userId)
            ->join('user', 'user.id', '=', 'user_relation.children_id')
    ...
    public static function getOneFinishTotal($userId)
    {
        $result = DB::table('order')
            ->where('order.status_id', Config::get('constants.orderStatus.finish'))
    ...
    public static function getSecondFinishTotal($userId)
    {
        $result = DB::table('order')
            ->where('order.status_id', Config::get('constants.orderStatus.finish'))
            ->whereIn('order.user_id', function ($query) use ($userId) {
